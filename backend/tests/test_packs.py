...
from tests.util import make_spec
...
def test_create_pack_and_version(client):
    resp = client.post("/packs", json={"name": "Vendor Due Diligence"})
    assert resp.status_code == 201
    pack_id = resp.json()["id"]

    resp = client.post(f"/packs/{pack_id}/versions", json={"spec": SPEC.model_dump()})
    assert resp.status_code == 201
    body = resp.json()
...
def test_next_version_is_incremental(client):
    resp = client.post("/packs", json={"name": "P"})
    pack_id = resp.json()["id"]
    for _ in range(2):
        client.post(f"/packs/{pack_id}/versions", json={"spec": SPEC.model_dump()})
