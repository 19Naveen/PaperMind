...
import { ActionButton, Input, Tag } from '@/components/ui';
import { IconAlert, IconCheck } from '@/lib/icons';
import { changePasswordAction, updateMeAction, type ProfileFailure } from '@/lib/session';

/**
 * The interactive half of the account page. `/settings` itself is a server
 * component (it renders the server-side activity counters), so the cards that
 * hold client state live here and are composed into its grid.
 */
...
/**
 * Identity and the fields that change it, in one card. This is where the old
 * /profile identity block landed: repeating the avatar, name and role in a
 * second read-only card would rebuild, inside one page, the duplication the
 * merge set out to remove.
 */
    ...
    <div className="card set-card">
      ...
      <div className="avatar-row">
        ...
        <div className="prof-id">
          <h1>
            {user?.name}
            {roleLabel && <Tag variant="accent">{roleLabel}</Tag>}
          </h1>
          <p>{user?.email}</p>
        </div>
      </div>
